<?php
// programa relacionado con registro_espectadores
...
if ($tam != 16) {
    ...
    if ($existe_espectador == 0) { // 0 indica que no existe espectador previamente
        ...
        $insertado = registrar_espectador($nombre, $DNI, $tarjeta);
        header("Location: http://localhost/cine/registro_espectadores.php?mensaje=Espectador insertado con el DNI: " . $DNI);
    } else { // ya existe el espectador

        header("Location: http://localhost/cine/registro_espectadores.php?mensaje=espectador ya registrado con el DNI: " . $DNI);
    }
}
